...
class FreshRSSAPI:
    ...
    def get_items(self, since_id: Union[str, int, None] = None, max_id: Union[str, int, None] = None,
                  with_id: Optional[list[int]] = None) -> list[Item]:
        """
    ...
    def get_all_items(self, timeout: int = 300, verbose: bool = False, 
                      n_max: Optional[int] = None) -> list[Item]:
        """
        ...
        while True:
            ...
            if verbose:
                print(f"Found {len(items)} new items. Total items so far: {len(all_items)}")
                if n_max:
